...
# from usl.qwen.split_config import SplitModelConfig
...
class QwenSplitModel(nn.Module):
    def __init__(self, qwen:Qwen2PreTrainedModel,split_config:SplitModelConfig,dp_config:DPConfig=DPConfig(),add_lora=False,lora_config:LoraConfig=None,*args, **kwargs):
        super().__init__()
    ...
    def _register_grad_hooks(self):
        ...
        self._activation_and_grad_hooks.append(self.server_model.layers[0].register_full_backward_hook(grad_wrapper(name=Intermediate.SERVER2HEAD_GRADIANT)))
        self._activation_and_grad_hooks.append(self.tail_model.layers[0].register_full_backward_hook(grad_wrapper(name=Intermediate.TAIL2SERVER_GRADIANT)))
...
class QwenHead(SplitModel):
    ...
    def _load_weight_from_pretrained_model_physically(self, pretrained_model:Qwen2ForCausalLM,from_l,to_l):
        ...
        hidden_layers:List[Qwen2DecoderLayer]
        self.layers = nn.ModuleList(
            [Qwen2DecoderLayer(self.config, layer_idx) for layer_idx in range(to_l-from_l)]
        )
        for i in range(from_l,to_l):
    ...
    def forward(
        ...
        **kwargs
        ...
        causal_mask = _update_causal_mask(
            self,attention_mask, inputs_embeds, cache_position, past_key_values, output_attentions
        )
        #--------------------------------------------------------------------------变量处理完毕，准备前向传播
        ...
        return hidden_states,causal_mask,position_ids,\
                ...
                all_hidden_states,all_self_attns,return_legacy_cache
...
class QwenServer(SplitModel):
    ...
    def _load_weight_from_pretrained_model_physically(self, pretrained_model:Qwen2ForCausalLM,from_l,to_l):
        ...
        hidden_layers:List[Qwen2DecoderLayer]
        self.layers = nn.ModuleList(
            [Qwen2DecoderLayer(self.config, layer_idx+to_l) for layer_idx in range(to_l-from_l)]
        )
        for i in range(from_l,to_l):
            self.layers[i-from_l].load_state_dict(hidden_layers[i].state_dict())
...
class QwenTail(SplitModel):
    ...
    def _add_nosie_on_grad_from_tail_to_server(self):  
        ...
        # self.layers = nn.ModuleList(
        #     # [Qwen2DecoderLayer(config, layer_idx) for layer_idx in range(split_config.tail_layer_num)]
        # )
...
#首先从与训练模型中加载模型
def split_qwen(pretrained_model:Qwen2ForCausalLM,split_config:SplitModelConfig,dp_config:DPConfig)->Tuple[QwenHead,Optional[QwenServer],QwenTail]:
    # self.split_config = split_config
