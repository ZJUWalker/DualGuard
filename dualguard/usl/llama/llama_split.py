...
class LlamaSplitModel(nn.Module):
    def __init__(self, llama:LlamaForCausalLM,split_config:SplitModelConfig,dp_config:DPConfig=DPConfig(),add_lora=False,lora_config={},*args, **kwargs):
        super().__init__()
...
class LlamaHead(SplitModel):
    ...
    def _load_weight_from_pretrained_model_physically(self, pretrained_model:LlamaForCausalLM,from_l,to_l):
        ...
        hidden_layers:List[LlamaDecoderLayer]
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer(self.config, layer_idx) for layer_idx in range(to_l-from_l)]
        )
        for i in range(from_l,to_l):
...
class LlamaServer(SplitModel):
    ...
    def _load_weight_from_pretrained_model_physically(self, pretrained_model:LlamaForCausalLM,from_l,to_l):
        ...
        hidden_layers:List[LlamaDecoderLayer]
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer(self.config, layer_idx+to_l) for layer_idx in range(to_l-from_l)]
        )
        for i in range(from_l,to_l):
            self.layers[i-from_l].load_state_dict(hidden_layers[i].state_dict())
...
#首先从与训练模型中加载模型
def split_llama(pretrained_model:LlamaForCausalLM,split_config:SplitModelConfig,dp_config:DPConfig)->Tuple[LlamaHead,Optional[LlamaServer],LlamaTail]:
    # self.split_config = split_config
