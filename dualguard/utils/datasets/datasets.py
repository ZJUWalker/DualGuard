...
from dualguard.utils.exp import register_dataset
from dualguard.utils import env as env_config
...
class PIQAFedDataset(FedDataset):
    ...
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac: float = 0.3, **kwargs):
        super().__init__(tokenizer, client_ids, dataset=load_dataset(env_config.dataset_cache_dir + 'piqa'),
...
class STSBFedDataset(FedDataset):
    ...
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac: float = 0.3, **kwargs):
        super().__init__(tokenizer, client_ids, dataset=load_dataset(env_config.dataset_cache_dir + 'stsb'),
                         types=['train', 'test', 'validation'],
                         shrink_frac=shrink_frac, **kwargs)
...
class QNLIFedDataset(FedDataset):
    ...
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac: float = 0.3, **kwargs):
        super().__init__(tokenizer, client_ids, dataset=load_dataset(env_config.dataset_cache_dir + 'qnli'),
                         types=['train', 'test', 'validation'],
                         shrink_frac=shrink_frac, **kwargs)
        self.q_temp = "### Question:\n"
        self.a_temp = "### Solution:\n"
...
class MRPCFedDataset(FedDataset):
    ...
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac: float = 0.3, **kwargs):
        super().__init__(tokenizer, client_ids, dataset=load_dataset(env_config.dataset_cache_dir + 'mrpc'),
                         types=['train', 'test', 'validation'],
                         shrink_frac=shrink_frac, **kwargs)
        self.q_temp = "### Question:\n"
        self.a_temp = "### Solution:\n"
...
class CoLAFedDataset(FedDataset):
    ...
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac: float = 0.3, **kwargs):
        super().__init__(tokenizer, client_ids, dataset=load_dataset(env_config.dataset_cache_dir + 'cola'),
                         types=['train', 'test'],
                         shrink_frac=shrink_frac, **kwargs)
...
class RTEFedDataset(FedDataset):
    ...
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac: float = 0.3, **kwargs):
        super().__init__(tokenizer, client_ids, dataset=load_dataset(env_config.dataset_cache_dir + 'rte'),
                         types=['train', 'test', 'validation'],
                         shrink_frac=shrink_frac, **kwargs)
        self.q_temp = "### Question:\n"
        self.a_temp = "### Solution:\n"
...
class CodeAlpacaFedDataset(FedDataset):
    ...
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac: float = 0.3, **kwargs):
        super().__init__(tokenizer, client_ids, load_dataset(env_config.dataset_cache_dir + 'CodeAlpaca_20K'),
                         ['train', 'test'],
                         shrink_frac, **kwargs)
...
class E2EDataset(FedDataset):
    def __init__(self, tokenizer, client_ids: list[str], shrink_frac: float = 0.3, **kwargs):
        super().__init__(tokenizer, client_ids, dataset=load_dataset(env_config.dataset_cache_dir + 'e2e'),
    ...
    def _col_fun(self, batch, max_seq_len=-1, extra_info=True):
        ...
        input = self.tokenizer(texts, padding=True, truncation=True, max_length=max_len,
                               return_tensors='pt')
