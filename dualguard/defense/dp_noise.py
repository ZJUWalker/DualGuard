import math
import numpy as np
from transformers import AutoModelForCausalLM,LlamaForCausalLM,Qwen2ForCausalLM,GPT2LMHeadModel
...
def gamma_simulation(num_simulation=100000,model:AutoModelForCausalLM=None):
    # model = BertForMaskedLM.from_pretrained("bert-base-uncased"
    # embedding_matrix = model.bert.embeddings.word_embeddings.weight.data.cpu().numpy()
    embedding_matrix = _get_embedding_(model).weight.data.cpu().numpy()
...
def cal_sensitivity_bert_embedding(model:AutoModelForCausalLM=None):
    """
...
def _get_embedding_(model:AutoModelForCausalLM=None)->torch.nn.Embedding:
    # if isinstance(model, (LlamaForCausalLM,Qwen2ForCausalLM)):
    #     return model.get_input_embeddings()
    # elif isinstance(model, GPT2LMHeadModel):
    #     return model.get_input_embeddings()
    if model.__class__.__dict__.get('get_input_embeddings') is not None:
        return model.get_input_embeddings()
    else:
        raise NotImplementedError('no get_input_embeddings method found for model type')


def get_noise_multiplier(eps, delta, batch_size=1, dataset_size=50000, epoch=3, local_dp=False, noise_type='GM'):
    if local_dp:
        if noise_type == 'aGM':
...
def add_noise(embeddings:torch.Tensor, noise_factor=0.5,norm_c=1.0,add_noise=True,in_place=True):
    # print(noise_factor)
    # noise_factor=0.1
    if add_noise:
        if in_place:
            embeddings = _max_norm_clip(embeddings, norm_c)
            noise_embeds = noise_factor * torch.normal(mean=0, std=1e-3, size=embeddings.shape).to(embeddings.device)
            embeddings = noise_embeds + embeddings
            return embeddings
        else:
            new_embeddings = _max_norm_clip(embeddings, norm_c)
            noise_embeds = noise_factor * torch.normal(mean=0, std=1e-3, size=new_embeddings.shape).to(new_embeddings.device)
            new_embeddings = noise_embeds + new_embeddings
            return new_embeddings
    return embeddings


def _max_norm_clip(embeddings:torch.Tensor, norm_c=1.0):
