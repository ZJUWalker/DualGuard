...
from dualguard.attack import tag,lamp
from typing import Union
    ...
    **kwargs
    ...
    with tqdm(total=iters) as pbar:
        for i in range(iters):
            ...
            # dummy_activation_x=dummy_activation_x.clone().detach().requires_grad_(True)
            ...
            pbar.set_description(
                f'Iter: {i}/{iters}  matching Loss: {loss.item()}')
            pbar.update(1)
...
def bisr_attack(
    ...
    if is_warmup_usl:
        dummy_x,true_grads,tail_output_logits,attention_mask,position_ids=tag.forward_and_get_true_grads(head_model,server_model,tail_model,batch,device,forzen_tail_model)
    else:
        dummy_x,true_grads,tail_output_logits,attention_mask,position_ids=tag.forward_and_get_true_grads(head_model,server_model,tail_model,batch,device)
    ...
    dummy_embedding_x=sma_attack(
        ...
        dummy_label_x=dummy_x_tokens,
        real_activation_x=h2s_ac,
        optimizer=torch.optim.Adam,
        iters=sma_iters  
    )
