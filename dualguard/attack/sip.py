...
class GRUDRInverter(nn.Module):
    ...
    def __init__(self,n_embed,vocab_size,hidden_size=256,bidirectional=True, *args, **kwargs):
        super().__init__()
        self.gru = nn.GRU(input_size=n_embed, hidden_size=hidden_size, batch_first=True,
        ...
        for epc in range(sip_args.epochs):
            ...
            for step, batch in enumerate(dataloader):
                ...
                pbar.set_description(
                    f'Epoch {epc+1} | Loss {loss.item():.5f} | Rouge_Lf1 {rouge_l_f / (step + 1):.4f}')
                pbar.update(1)
...
def sip_model_evaluate(env_args:EnvArgs, attack_model:nn.Module, head_model:SplitModel,tokenizer:AutoTokenizer, valid_loader:DataLoader=None,logger:Logger=None):
    device = env_args.device
    ...
    total_steps = len(dataloader)  # 获取总步数
    _loss = 0
    # 创建 tqdm 进度条
    with tqdm(total=total_steps, desc="Training Progress") as pbar:
        for step, batch in enumerate(dataloader):
            ...
            meteors += meteor
            pbar.set_description(
                f'Loss: {_loss / (step + 1):.5f} | Rouge_Lf1: {rouge_l_f / (step + 1):.4f} | Meteor: {meteors / (step + 1):.4f}'
            )
            pbar.update(1)  
