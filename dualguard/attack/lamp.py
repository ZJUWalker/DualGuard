    ...
    **kwargs,
    ...
    with tqdm(total=attack_iters, desc='LAMP Attack') as pbar:
        for i in range(attack_iters):
            optim.zero_grad()
            loss, grad_diff = _calcuate_loss(
                beta, forzen_llm, pt_llm, dummy_x, true_grads, dummy_lables_logits, with_ppl=False, **kwargs
            )  # loss仅用于打印
            grad_diff.backward()
            optim.step()
            if i % lamp_freq == 0:
                new_dummy_lables_logits = _lamp(
                    lamp_step, beta, forzen_llm, pt_llm, dummy_x, true_grads.clone().detach(), dummy_lables_logits, with_ppl=False, **kwargs
                )
                with torch.no_grad():
...
def attack(
    ...
    dummy_x, true_grads, tail_output_logits, attention_mask, position_ids = forward_and_get_true_grads(
        model_head, model_server, model_tail, batch, device
    )
