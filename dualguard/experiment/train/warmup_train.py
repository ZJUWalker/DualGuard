...
def warmup_train(
    ...
    **kwargs,
    ...
    # 设置优化器
    head_optimizer = wp_args.optimizer(
        head_model.parameters(), lr=wp_args.optimizer_kwargs['lr'], weight_decay=wp_args.optimizer_kwargs['weight_decay']
    )
    tail_optimizer = wp_args.optimizer(
        tail_model.parameters(), lr=wp_args.optimizer_kwargs['lr'], weight_decay=wp_args.optimizer_kwargs['weight_decay']
    )
    # 设置辅助模型
    hidden_size = 0
    if isinstance(head_model, (LlamaHead, QwenHead)) or (
        isinstance(head_model, PeftModelForCausalLM) and isinstance(head_model.base_model.model, (LlamaHead, QwenHead))
    ):
    ...
    pmlp_optimizer = wp_args.warm_up_optimizer(
        project_mlp.parameters(), lr=wp_args.warm_up_optimizer_kwargs['lr'], weight_decay=wp_args.warm_up_optimizer_kwargs['weight_decay']
    )
    head_model.train()
    tail_model.train()
    project_mlp.train()
    ...
    for epoch in range(wp_args.warm_up_epochs):
        ...
        for idx, batch in enumerate(train_loader):
            ...
            bwd_loss = wp_args.attack_loss_w / attack_loss + wp_args.pt_loss_w / pt_tail_loss
            ...
            if (idx + 1) % wp_args.log_interval == 0:
                log_str = (
                    ...
                    f"| lm_loss: {total_lm_loss /wp_args.log_interval:.4f} | attack_loss: {total_attack_loss / wp_args.log_interval:.4f} "
                    f"| lm_loss_before: {total_pt_tail_loss / wp_args.log_interval:.4f} | totol_loss: {total_loss / wp_args.log_interval:.4f}"
            ...
            if (idx + 1) % validation_interval == 0:
                valid_total_loss, valid_lm_loss, valid_attack_loss, valid_lm_loss_before = warmup_validation(
                    wp_args, head_model, project_mlp, tail_model, pt_tail_model, attack_model, valid_loader
                )
                log_str = (
                    f"| Eval {(idx+1) // validation_interval:3d} at step {(idx+1):>8d} | "
                    f"valid lm_loss {valid_lm_loss:5.3f} | "
                    f"valid attack_loss {valid_attack_loss:5.3f} | valid_lm_loss_before {valid_lm_loss_before:5.3f} | valid total loss {valid_total_loss:5.3f}"
                )
                logger.info(log_str)
                print(log_str)
                head_model.train()
                ...
                attack_model.train()
                pt_tail_model.train()
    return head_model, tail_model, project_mlp
    pass
...
def _warmup_forward(
    ...
    # 获取warmup的中间结果和loss
    if isinstance(tail_model, (LlamaTail, QwenTail)) or (
        isinstance(tail_model, PeftModelForCausalLM) and isinstance(tail_model.base_model.model, (LlamaTail, QwenTail))
    ):
        (
            hidden_states_from_head,
            causal_mask,
        ...
        tail_output = tail_model.forward(
            hidden_status_from_server=hidden_states_from_head,
    ...
    # print(f'pt loss is {pt_tail_loss}')
...
def warmup_validation(
    ...
    with torch.no_grad():
        for idx, data in enumerate(valid_loader):
            ...
            tail_output = {}
            # 获取warmup的中间结果和loss
            if isinstance(head, (LlamaHead, QwenHead)) or (
                isinstance(head, PeftModelForCausalLM) and isinstance(head.base_model.model, (LlamaHead, QwenHead))
            ):
                (
                    hidden_states_from_head,
                    causal_mask,
            ...
            # 获取经过sip模型的输出loss
            attack_logits = attack_model(hidden_states_from_head)
            if isinstance(tail, (LlamaTail, QwenTail)) or (
                isinstance(tail, PeftModelForCausalLM) and isinstance(tail.base_model.model, (LlamaTail, QwenTail))
            ):
                pt_tail_output = pt_tail_model.forward(
                    hidden_status_from_server=hidden_states_from_head,
                    attention_mask=causal_mask,
        ...
        return (
            sum(_total_losses) / len(_total_losses),
            sum(_lm_losses) / len(_lm_losses),
            sum(_attack_losses) / len(_attack_losses),
            sum(_lm_losses_before) / len(_lm_losses_before),
        )


def _load_split_model_from_pretrained(
    warmup_args: WarmupArgs, split_config: SplitModelConfig
) -> Tuple[Union[LlamaSplitModel, QwenSplitModel, GPT2SplitModel], AutoTokenizer]:
...
def _load_models_and_tokenizer(wp_args: WarmupArgs, env_args: EnvArgs):
    ...
    split_model, tokenizer = _load_split_model_from_pretrained(
        wp_args, SplitModelConfig(split_layer_num, -1, split_layer_num, False, True)
    )  # warmup训练不需要server
...
if __name__ == '__main__':
    ...
    # 配置日志记录
    filename = f'warmup_training_{simple_model_name}_{ds_args.dataset_name}_no_lm_loss.log'  # 日志文件名
    logger = create_logger(log_args=LogArgs(log_dir=env.warmup_log_dir, log_file_name=filename))
    logger.info(f"\n{'='*48} warmup training on model {simple_model_name} on dataset {ds_args.dataset_name} with no lm loss {'='*48}")
    # 加载模型和tokenizer
    ...
    # 训练模型
    wramup_head_model, wramup_tail_model, mlp_model = warmup_train(
        wp_args=wp_args,
        env_args=env_args,
    ...
    head_ph = os.path.join(save_dir, f"head.pth")
    tail_ph = os.path.join(save_dir, f"tail.pth")
