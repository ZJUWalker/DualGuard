    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            if layer_name in name:
                print(
                    f'{name},grad:{param.grad[0,:length].detach().cpu().numpy() if param.grad is not None else "no grad"},param:{param[0,:length].detach().cpu().numpy()}'
                )
                break
    ...
    for idx, batch in enumerate(train_loader):
        ...
        if isinstance(head_model, GPT2Head) or (isinstance(head_model, PeftModelForCausalLM) and isinstance(head_model.base_model.model, GPT2Head)):
            ...
            tail_outputs = tail_model(
                hidden_status_from_server=hidden_states_from_server,
                presents=presents,
                past_key_values=past_key_values,
                attention_mask=attention_mask,
                head_mask=head_mask,
...
def evaluate_usl(
    ...
    with torch.no_grad():
        for idx, batch in enumerate(valid_loader):
            ...
            if isinstance(head_model, GPT2Head) or (
                isinstance(head_model, PeftModelForCausalLM) and isinstance(head_model.base_model.model, GPT2Head)
            ):
                temp = head_model(input_ids=_input, attention_mask=_attention_mask)
                (
                    hidden_states,
                    presents,
                    past_key_values,
                ...
                temp = server_model(
                    hidden_status_from_head=hidden_states,
                    presents=presents,
                    past_key_values=None,
                    attention_mask=attention_mask,
                    head_mask=head_mask,
                ...
                tail_outputs = tail_model(
                    hidden_status_from_server=hidden_states,
                    presents=presents,
                    past_key_values=None,
                    attention_mask=attention_mask,
                    head_mask=head_mask,
...
def _load_warmup_model_and_tokenizer(usl_args: USLTrainArgs, dataset_args: DatasetArgs, env_args: EnvArgs, dp_config: DPConfig = None):
    ...
    if not usl_args.use_naive_usl:
        ...
        split_model.head_model = torch.load(
            os.path.join(wp_model_dir, f'{simple_name}/{dataset_args.dataset_name}/head_40_80_no_mlp.pth'),
            map_location=env_args.device,
            weights_only=False,
        )
        split_model.tail_model = torch.load(
            os.path.join(wp_model_dir, f'{simple_name}/{dataset_args.dataset_name}/tail_40_80_no_mlp.pth'),
            map_location=env_args.device,
            weights_only=False,
        )
...
def _print_trainable_parameters(model: nn.Module):
    ...
    print(
        f'trainable params count:{trainable_params_count}, total params count:{total_prams_count},ratio:{trainable_params_count/total_prams_count:.3f}'
    )


total_args = [dualguard_config]  # flexable args
...
if __name__ == '__main__':
    ...
    for args in total_args:
        ...
        # 配置日志记录
        logger = create_logger(LogArgs(log_dir=env.usl_log_dir, log_file_name=f'{simple_name}_{args.prefix}_{ds_args.dataset_name}_nomlp.log'))
        ...
        # # 验证
        avg_loss, ppl, rouge_l_f, meteor = evaluate_usl(
            usl_args, env_args, head_model, server_model, tail_model, valid_data_loader, tokenizer, output_similarity=True
        )
        ...
        if not usl_args.frozen_head:
            torch.save(usl_head_model, f"{save_dir}/{save_prefix}_head_nomlp.pth")
        torch.save(usl_server_model, f"{save_dir}/{save_prefix}_server_nomlp.pth")
        torch.save(usl_tail_model, f"{save_dir}/{save_prefix}_tail_nomlp.pth")
