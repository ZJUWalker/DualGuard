    ...
    for args in total_args:
        ...
        try:
            head_model,server_model,tail_model,forzen_head_model,forzen_tail_model,tokenizer\
                =load_needed_models(args,ds_args,env_args,dp_config)
            #加载数据集
            ...
            if AttackMethod.TAG_ATTACK in eval_methods:
                ...
                rouge_l_f,meteor=tag_attack_evaluate(
                    env_args=env_args,
                    tag_args=TAGAttackArgs(sample_num=2,attack_iters=500,is_warmup_usl=True) if method_prefix == 'warmup_usl' else TAGAttackArgs(sample_num=2,attack_iters=500),
                    head_model=head_model,
