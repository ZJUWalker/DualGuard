...
from dualguard.utils.configs import LogArgs, DatasetArgs, EnvArgs, TAGAttackArgs, LampAttackArgs, BISRAttackArgs
...
from dualguard.utils.env import sip_model_dir
...
def dataset_len(dataloader: DataLoader):
    ...
    return {'length': len(dataloader), 'avg_token_len': token_len / len(dataloader)}


total_args = [dualguard_config]  # 可拔插的配置
eval_methods = [AttackMethod.CONNECT_TO_PT_TAIL]  # 可拔插的评估方法
...
if __name__ == '__main__':
    ...
    for args in total_args:
        ...
        usl_args.model_name = target_model_name
        ...
        sip_abs_path = os.path.join(sip_model_dir, f'sip_{simple_name}.pth')
        logger = create_logger(
            log_args=LogArgs(log_dir=env.eval_log_dir, log_file_name=f'usl_defense_eval_{simple_name}_on_{ds_args.dataset_name}.log')
        )
        ...
        # load needed models
        try:
            head_model, server_model, tail_model, forzen_head_model, forzen_tail_model, tokenizer = load_needed_models(
                args, ds_args, env_args, dp_config
            )
            # load dataset
            ...
            if AttackMethod.SIP_ATTACK in eval_methods:
                # tag_attack
                ...
                rouge_l_f, meteor, loss = sip_attack_evaluate(
                    env_args=env_args, head_model=head_model, attack_model=sip_model, tokenizer=tokenizer, data_loader=valid_data_loader
                )
            ...
            if AttackMethod.TAG_ATTACK in eval_methods:
                ...
                rouge_l_f, meteor = tag_attack_evaluate(
                    env_args=env_args,
                    tag_args=(
                        TAGAttackArgs(sample_num=1, attack_iters=500, is_warmup_usl=True)
                        if method_prefix == 'warmup_usl'
                        else TAGAttackArgs(sample_num=1, attack_iters=500)
                    ),
                ...
                print(log_str)
                torch.cuda.empty_cache()
            if AttackMethod.LAMP_ATTACK in eval_methods:
                # lamp_attack
                ...
                rouge_l_f, meteor = lamp_attack_evaluate(
                    env_args=env_args,
                    lamp_args=(
                        LampAttackArgs(sample_num=1, attack_iters=1500, lamp_interval=200, is_warmup_usl=True)
                        if method_prefix == 'warmup_usl'
                ...
                torch.cuda.empty_cache()
            # bisr_attack
            if AttackMethod.BISR_ATTACK in eval_methods:
                ...
                rouge_l_f, meteor = bisr_attack_evaluate(
                    env_args=env_args,
                    bisr_args=(
                        BISRAttackArgs(sample_num=1, attack_iters=1200, is_warmup_usl=True)
                        if method_prefix == 'warmup_usl'
                ...
                logger.info(log_str)
                print(log_str)
            if AttackMethod.CONNECT_TO_PT_TAIL in eval_methods:
                # directly connect to pt_tail
                log_str = f'connet-to-pt-tail attack on model {simple_name} on attack method {method_prefix} and dataset {ds_args.dataset_name}'
                logger.info(log_str)
                print(log_str)
                loss, ppl, rouge_l_f, meteor = connet_to_pt_tail_evaluate(
                    env_args=env_args,
                    head_model=head_model,
                    server_model=server_model,
                    pt_tail_model=forzen_tail_model,
                    tokenizer=tokenizer,
                    data_loader=valid_data_loader,
                )
